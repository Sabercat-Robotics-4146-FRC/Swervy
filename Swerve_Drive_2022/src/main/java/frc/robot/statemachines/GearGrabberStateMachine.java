...
public class GearGrabberStateMachine {
  private static final double kGearIntakePower = -1.0;
  private static final double kGearExhaustPower = 1.0;

  private static final double kStowDelayTime = 1.0; // sec to keep intaking while in stowed position
  private static final double kShootDelayTime =
      0.1; // sec to keep gear while grabber moves down before shooting
  private static final double kStowingDelayTime =
      0.5; // sec to stay in stowing before shifting stowed position

  private static final boolean kSolenoidUpValue = true;
  ...
  public synchronized GearGrabberState update(
      double timestamp, WantedState wantedState, GearGrabberState currentState) {
  ...
  private SystemState handleIntakeTransition(
      WantedState wantedState, GearGrabberState currentState) {
    switch (wantedState) {
      case IDLE:
        return SystemState.STOWING;
      default:
